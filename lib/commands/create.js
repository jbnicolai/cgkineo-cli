        ...
        confirmType({
            ...
            case "course":
                ...
                .then(function (properties) {
                    ...
                    var from  = path.join(to, "/**/.gitignore");
                    renderer.log('\nremoving .gitignore files from ' + to + '.\n');
                    var tasks = 0;
                    var cb = function() {
                        tasks++;
                        if (tasks == 2) deferred.resolve(properties);
                    };

                    var str1 = gulp.src(from);
                        str1.pipe(grename(function (path) {
                            path.basename = ".gtignr";
                            path.extname = ".bak";
                        }))
                        .pipe(gulp.dest(to));
                        str1.on('end', cb);

                    var str2 = gulp.src(from);
                        str2.pipe(grimraf());
                        str2.on('end', cb);
